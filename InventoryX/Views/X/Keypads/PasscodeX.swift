...
@MainActor class PasscodeXViewModel: ObservableObject {
    ...
    @Published var processes: [PasscodeXState]
    @Published var passcodeHash: String = ""
    let onSuccess: (() -> Void)
    
    init(processes: [PasscodeXState], onSuccess: @escaping (() -> Void)) {
...
struct PasscodeX: View {
    @Environment(\.dismiss) var dismiss
    @StateObject var vm: PasscodeXViewModel
    let constProc: [PasscodeXState]
    ...
    init(processes: [PasscodeXState], onSuccess: @escaping (() -> Void)) {
        constProc = processes
        self._vm = StateObject(wrappedValue: PasscodeXViewModel(processes: processes, onSuccess: onSuccess))
    }
    ...
    private var verLayout: some View {
        VStack {
            ...
            KeypadView(strValue: $passcode, tapKey: { key in
